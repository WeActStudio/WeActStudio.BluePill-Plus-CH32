...
#define FLASH_Fast_Test_Addr (0x0800ff00)
    FLASH_ErasePage_Fast(FLASH_Fast_Test_Addr);

    printf("128Byte Page Erase Sucess\r\n");

    FLASH_BufReset();
    FLASH_BufLoad(FLASH_Fast_Test_Addr, buf[0], buf[1], buf[2], buf[3]);
    FLASH_BufLoad(FLASH_Fast_Test_Addr + 0x10, buf[4], buf[5], buf[6], buf[7]);
    FLASH_BufLoad(FLASH_Fast_Test_Addr + 0x20, buf[8], buf[9], buf[10], buf[11]);
    FLASH_BufLoad(FLASH_Fast_Test_Addr + 0x30, buf[12], buf[13], buf[14], buf[15]);
    FLASH_BufLoad(FLASH_Fast_Test_Addr + 0x40, buf[16], buf[17], buf[18], buf[19]);
    FLASH_BufLoad(FLASH_Fast_Test_Addr + 0x50, buf[20], buf[21], buf[22], buf[23]);
    FLASH_BufLoad(FLASH_Fast_Test_Addr + 0x60, buf[24], buf[25], buf[26], buf[27]);
    FLASH_BufLoad(FLASH_Fast_Test_Addr + 0x70, buf[28], buf[29], buf[30], buf[31]);
    FLASH_ProgramPage_Fast(FLASH_Fast_Test_Addr);
    ...
    for (i = 0; i < 32; i++)
    {
        if (buf[i] == *(u32 *)(FLASH_Fast_Test_Addr + 4 * i))
        {
            Verity_Flag = 0;
        }
        else
